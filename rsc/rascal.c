...
void initialize_lisp() {
  ...
  puts_env(new_sym("%"), &GLOBALS, new_prim(prim_mod, 2));
  puts_env(new_sym("pow"), &GLOBALS, new_prim(prim_pow, 2));
  ...
  puts_env(new_sym("def"), &GLOBALS, new_form(form_def, 2));
  puts_env(new_sym("setq"), &GLOBALS, new_form(form_setq, 2));
  puts_env(new_sym("quote"), &GLOBALS, new_form(form_quote, 1));
  puts_env(new_sym("if"), &GLOBALS, new_form(form_if, 3));
  puts_env(new_sym("fn"), &GLOBALS, new_form(form_fn, 2));
  puts_env(new_sym("do"), &GLOBALS, new_form(form_do, 1));
